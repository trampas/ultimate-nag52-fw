#!/usr/bin/env python3
"""Execute the actual bank-switching scheduler through 6EA2 versus native C."""
import ctypes as c
...
def main():
    with tempfile.TemporaryDirectory(prefix='egs51-scheduler-') as td:
        ...
        lib=c.CDLL(str(so));fn=lib.egs51_scheduler_prefix_tick;fn.argtypes=[c.POINTER(State)];fn.restype=None
        cpu=base(0)
        ...
        ims=[0x20,0x23,0x35,0x37,*range(0x44,0x58),*range(0x80,0x9b),*range(0xa9,0xba)]
        visited=set();modes=set()
        for tick in range(1800):
            # Scripted decoded inputs and gear demand, not an OEM shift-point
            # selector or physical transmission plant. State is independently retained.
            desired=[3,2,1,3,5,4][(tick//300)%6]
            for st in (cpu,native):
                st.x[0x75]=desired;st.im[0xab]=desired
            ...
            try:
                cpu.run(0x6e6a,stop=0x6ea3,stop_bank=0);fn(c.byref(native))
                assert cpu.pc==0x6ea3 and cpu.bank==0 and cpu.direct(0x81)==0xc0
        ...
        assert {0x0ba3,0x0ba8,0x0bad,0x0bb2,0x0bb7,0x0bbc}<=visited
        print(f'PASS: 1800 persistent original scheduler prefixes (34200 ordered stage calls); modes {sorted(modes)}',flush=True)
