...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='egs51-interrupt-') as td:
        ...
        routines=[('interrupt_fill_capacity',0x875d,[U8]),('interrupt_release_pressure',0x877a,[]),
                  ('interrupt_release_mod_pressure',0x8836,[U8,U16]),('interrupt_release_settle_pressure',0x89b5,[U8]),
                  ('interrupt_transfer_mod_pressure',0x8a92,[U8]),('interrupt_transfer_pressure',0x8b0c,[]),
                  ('interrupt_release',0x9f91,[]),('interrupt_release_transfer',0xa3a5,[])]
        import os
        ...
        for name,entry,types in routines:
            ...
            for j in range(6000):
                ...
                cpu.im[0xb6]=rng.choice([2,3]);cpu.im[0xac]=rng.randrange(1,5) if cpu.im[0xb6]==1 else rng.randrange(5,9)
                ...
                args=[]
                if types:
                    args=[rng.randrange(65536) if types[0]==U16 else rng.randrange(256 if name=='interrupt_fill_capacity' else 8)]
