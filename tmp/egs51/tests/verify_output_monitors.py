#!/usr/bin/env python3
"""OEM output fault monitors and supply/temperature band selection."""
import ctypes as c
...
ROUTINES=[('output_enable_monitor',0x57ec),('starter_output_monitor',0x5821),('supply_temperature_band',0x1fc6)]
    ...
    with tempfile.TemporaryDirectory(prefix='egs51-demand-load-') as td:
        ...
        for name,entry in ROUTINES:
            ...
            for j in range(16000):
                ...
                cpu.put(0x80,rng.randrange(256));cpu.put(0x96,rng.randrange(256))
                cpu.put(0x88,rng.randrange(256)&0xef);cpu.x[0x1b7]=rng.randrange(5)
                ...
                try:check_memory(cpu,native,IMS,range(1024),[0,8])
