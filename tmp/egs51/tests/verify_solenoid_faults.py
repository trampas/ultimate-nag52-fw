#!/usr/bin/env python3
"""Solenoid fault monitor including OEM terminal watchdog-halt outcome."""
import ctypes as c
...
ROUTINES=[('solenoid_fault_monitor',0x1ea1)]
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='egs51-demand-load-') as td:
        ...
        for name,entry in ROUTINES:
            fn=getattr(lib,'egs51_'+name);fn.argtypes=[c.POINTER(State)];fn.restype=c.c_uint8
            for j in range(16000):
                ...
                native=state(cpu);cpu.run(entry,stop=0x1fb2,stop_bank=0);result=fn(c.byref(native));visited.update(cpu.visited)
                assert result==(cpu.pc!=0x1fb2)
                if name in ('controller_init','pressure_init'):assert result==cpu.r(7)
                try:check_memory(cpu,native,IMS,range(1024),[0x20])
