#!/usr/bin/env python3
"""Complete platform fault manager, latching and fallback parity."""
import ctypes as c
...
ROUTINES=[('platform_diagnostics',0x4d40)]
IMS=[*range(8,16),*range(0x20,0x30),*range(0x80,0x9b),*range(0xa9,0xc0)]
def configured():
    ...
    with tempfile.TemporaryDirectory(prefix='egs51-demand-load-') as td:
        ...
        for name,entry in ROUTINES:
            ...
            for j in range(16000):
                ...
                cpu.put(0xe8,rng.randrange(256));cpu.put(0x80,rng.randrange(256))
                ...
                try:check_memory(cpu,native,IMS,range(1024),[0,0x18,0x68])
        ...
        cpu=configured();cpu.im[0x80]=120;cpu.x[0x1b7]=1
        native=state(cpu);ticks=lib.egs51_platform_timers;ticks.argtypes=[c.POINTER(State)]
        for tick in range(2400):
            for st in (cpu,native):
                for group in range(8):
                    st.im[0x22+group]=(1<<((tick//100+group)%8)) if tick%100<50 else 0
                st.x[0x79]=0 # emulate completion of the outer persistence service
            cpu.run(0x6cf8);ticks(c.byref(native))
            cpu.run(0x4d40);fn(c.byref(native))
            try:check_memory(cpu,native,IMS,range(1024),[0,0x18,0x68])
            except AssertionError as e:raise AssertionError(('persistent',tick,str(e))) from e
        print('PASS: 2400 persistent platform timer/fault-manager pairs',flush=True)
