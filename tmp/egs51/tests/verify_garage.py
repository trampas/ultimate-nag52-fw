...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='egs51-interrupt-') as td:
        ...
        routines=[('garage_select',0x575d,[]),('garage_release',0x5906,[]),('garage_prefill',0x59fc,[]),('garage_inhibit',0x5b9c,[]),('fault_pressure',0xa930,[])]
        import os
        ...
        for name,entry,types in routines:
            ...
            fn=getattr(lib,'egs51_'+name);fn.argtypes=[ptr]+types;fn.restype=None
            for j in range(6000):
                cpu=configured();cpu.x[0x32e]=rng.randrange(31);word(cpu.x,0x181,rng.randrange(1,65536))
                cpu.im[0xb6]=5;cpu.im[0xac]=rng.randrange(1,5) if cpu.im[0xb6]==1 else rng.randrange(5,9)
                ...
                cpu.im[0xab]=rng.randrange(9);cpu.x[0x334]=rng.randrange(9)
                args=[]
                if types:
                ...
                try:check_memory(cpu,s,IMS+[0x94,0x97,0x9a],range(1024))
